...
with st.sidebar:
    ...
    st.markdown("Index documents and chat with AI.")
    ...
    # Auto-reset vector database if provider/embedding model changes to prevent FAISS dimension mismatch
    if "last_provider" not in st.session_state:
        st.session_state.last_provider = provider
    elif st.session_state.last_provider != provider:
        st.session_state.vector_db = None
        st.session_state.indexed_files = []
        st.session_state.messages = []
        st.session_state.last_provider = provider
        st.toast(f"Switched to {provider}. Index reset for embedding compatibility.", icon="🔄")
    
    # Helper to check if key is valid (not empty and not a template placeholder)
    def is_valid_key(key: str) -> bool:
        if not key or not isinstance(key, str):
            return False
        k = key.strip()
        if not k or "your_" in k.lower() or "here" in k.lower() or k.lower() in ["xxx", "xxxx", "dummy"]:
            return False
        return len(k) > 10

    # Check if a valid API key is configured server-side (in .env or system env)
    raw_env_key = ""
    if provider == "Google Gemini":
        raw_env_key = os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY") or ""
    elif provider == "OpenAI":
        raw_env_key = os.getenv("OPENAI_API_KEY") or ""
        
    env_key = raw_env_key if is_valid_key(raw_env_key) else ""
        
    # Secure key handling: Use server key if valid, never show key text in UI
    if env_key:
        api_key = env_key
        st.success(f"🔒 {provider} API Connected (Server Secured)")
    else:
        # Fallback to manual entry if no valid environment key is set on server
        api_key = st.text_input(
            f"Enter {provider} API Key",
            type="password",
            help=f"Enter your real {provider} API key here or update your .env file."
        )
        if not is_valid_key(api_key):
            st.warning(f"Please enter a valid {provider} API Key above or update your `.env` file.")
            api_key = ""
            ...
            with st.spinner("Processing documents and embedding text..."):
                ...
                if docs:
                    ...
                    try:
                        ...
                        err_msg = str(e)
                        if "API_KEY_INVALID" in err_msg or "401" in err_msg or "invalid_api_key" in err_msg or "Incorrect API key" in err_msg:
                            st.error(f"❌ **Invalid {provider} API Key!** The API key was rejected by {provider}.\n\n`API Details: {err_msg}`")
                        elif "429" in err_msg or "insufficient_quota" in err_msg or "credit_balance_exhausted" in err_msg:
                            st.error(f"❌ **{provider} Quota Exhausted (Error 429)!** Your account has no remaining credits.\n\n`API Details: {err_msg}`")
                        elif "404" in err_msg or "NOT_FOUND" in err_msg:
                            st.error(f"❌ **{provider} Model / Key Error!**\n\n`API Details: {err_msg}`")
                        else:
                            st.error(f"❌ **{provider} Indexing Error**: `{err_msg}`")
                else:
...
"""
                    ...
                    # Ensure reply is a string (LangChain can return a list for multipart content)
...
# Top-level export definitions for WSGI/ASGI/Deployment server compatibility
app = st
application = st
handler = st
